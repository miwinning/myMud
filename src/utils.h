...
size_t sprintbit(bitvector_t vektor, const char *names[], char *result,
                 size_t reslen);
...
void column_list(struct char_data *ch, int num_cols, const char **list,
                 int list_length, bool show_nums);
...
#define CHECK_PLAYER_SPECIAL(ch, var)                                          \
  (*(((ch)->player_specials == &dummy_mob)                                     \
         ? (log("SYSERR: Mob using '" #var "' at %s:%d.", __FILE__, __LINE__), \
            &(var))                                                            \
         : &(var)))
#else
...
#define IS_MOB(ch)                                                             \
  (IS_NPC(ch) && GET_MOB_RNUM(ch) <= top_of_mobt && GET_MOB_RNUM(ch) != NOBODY)
...
#define ZONE_FLAGGED(rnum, flag)                                               \
  (IS_SET_AR(zone_table[(rnum)].zone_flags, (flag)))
 ...
 * SECT_INSIDE. */
#define SECT(room)                                                             \
  (VALID_ROOM_RNUM(room) ? world[(room)].sector_type : SECT_INSIDE)

/** Return the zone number for this room */
#define GET_ROOM_ZONE(room)                                                    \
  (VALID_ROOM_RNUM(room) ? world[(room)].zone : NOWHERE)
...
/** Warning: GET_ID is deprecated and you should use char_script_id,
 * obj_script_id, room_script_id */
...
#define GET_OLC_ZONE(ch)                                                       \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.olc_zone))
/** Currently unused?
 * @deprecated Currently unused? */
#define GET_LAST_OLC_TARG(ch)                                                  \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->last_olc_targ))
/** Currently unused?
 * @deprecated Currently unused? */
#define GET_LAST_OLC_MODE(ch)                                                  \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->last_olc_mode))
/** Retrieve command aliases for ch. */
#define GET_ALIASES(ch)                                                        \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->aliases))
/** Who ch last spoke to with the 'tell' command. */
#define GET_LAST_TELL(ch)                                                      \
...
#define GET_LAST_MOTD(ch)                                                      \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.lastmotd))
#define GET_LAST_NEWS(ch)                                                      \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.lastnews))
/** Get channel history i for ch. */
#define GET_HISTORY(ch, i)                                                     \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.comm_hist[i]))
/** Return the page length (height) for ch. */
#define GET_PAGE_LENGTH(ch)                                                    \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.page_length))
/** Return the page width for ch */
#define GET_SCREEN_WIDTH(ch)                                                   \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.screen_width))
/* Autoquests data */
/** Return the number of questpoints ch has. */
#define GET_QUESTPOINTS(ch)                                                    \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.questpoints))
/** Return the current quest that a player has assigned */
#define GET_QUEST(ch)                                                          \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.current_quest))
/** Number of goals completed for this quest. */
#define GET_QUEST_COUNTER(ch)                                                  \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.quest_counter))
/** Time remaining to complete the quest ch is currently on. */
#define GET_QUEST_TIME(ch)                                                     \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.quest_time))
/** The number of quests completed by ch. */
#define GET_NUM_QUESTS(ch)                                                     \
  CHECK_PLAYER_SPECIAL((ch),                                                   \
                       ((ch)->player_specials->saved.num_completed_quests))
/** The type of quest ch is currently participating in. */
#define GET_QUEST_TYPE(ch)                                                     \
  (real_quest(GET_QUEST((ch))) != NOTHING                                      \
       ? aquest_table[real_quest(GET_QUEST((ch)))].type                        \
       : AQ_UNDEFINED)

/** The current skill level of ch for skill i. */
#define GET_SKILL(ch, i)                                                       \
  CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.skills[i]))
...
#define GET_MOB_VNUM(mob)                                                      \
  (IS_MOB(mob) ? mob_index[GET_MOB_RNUM(mob)].vnum : NOBODY)
...
#define IS_IN_OLC(d)                                                           \
  ((STATE(d) >= FIRST_OLC_STATE) && (STATE(d) <= LAST_OLC_STATE))
...
#define _3RD_EXIT(ch, door)                                                    \
  (world[_2ND_EXIT(ch, door)->to_room].dir_option[door])
...
#define IS_DIAGONAL(dir)                                                       \
  (((dir) == NORTHWEST) || ((dir) == NORTHEAST) || ((dir) == SOUTHEAST) ||     \
   ((dir) == SOUTHWEST))
...
/** 1 if ch is human class, 0 if not. */
...
#define IS_HAPPYHOUR                                                           \
  ((IS_HAPPYEXP || IS_HAPPYGOLD || IS_HAPPYQP) && (HAPPY_TIME > 0))
