...
static size_t print_zone_to_buf(char *bufptr, size_t left, zone_rnum zone,
                                int listall);
...
/* Local Globals */
static struct recent_player *recent_list =
    NULL; /** Global list of recent players */
...
ACMD(do_send) {
  ...
  send_to_char(vict, "%s\r\n", buf);
  mudlog(CMP, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s sent %s: %s",
         GET_NAME(ch), GET_NAME(vict), buf);
    ...
    if ((target_mob = get_char_vis(ch, mobobjstr, &num, FIND_CHAR_WORLD)) !=
      ...
      if (IN_ROOM(target_obj) != NOWHERE)
        location = IN_ROOM(target_obj);
               ...
               IN_ROOM(target_obj->carried_by) != NOWHERE)
...
ACMD(do_goto) {
  ...
  if (ZONE_FLAGGED(GET_ROOM_ZONE(location), ZONE_NOIMMORT) &&
    ...
    if (cmd_room == rrnum) {
      ...
      case 'G':
        send_to_char(ch, "%sGive it %s [%s%d%s], Max : %d\r\n",
                     ZOCMD.if_flag ? " then " : "",
                     obj_proto[ZOCMD.arg1].short_description, cyn,
      ...
      case 'T':
        send_to_char(
            ...
            trig_index[ZOCMD.arg2]->proto->name, yel, cyn,
            trig_index[ZOCMD.arg2]->vnum, yel,
...
static void do_stat_room(struct char_data *ch, struct room_data *rm) {
  ...
  send_to_char(
      ch,
      "Zone: [%3d], VNum: [%s%5d%s], RNum: [%5d], IDNum: [%5ld], Type: %s\r\n",
  ...
  for (i = 0; i < DIR_COUNT; i++) {
    ...
    if (rm->dir_option[i]->to_room == NOWHERE)
      snprintf(buf1, sizeof(buf1), " %sNONE%s", CCCYN(ch, C_NRM),
               CCNRM(ch, C_NRM));
    else
    ...
    send_to_char(
        ch,
        "Exit %s%-5s%s:  To: [%s], Key: [%5d], Keywords: %s, Type: %s\r\n%s",
...
static void do_stat_object(struct char_data *ch, struct obj_data *j) {
  ...
  send_to_char(
      ch,
      "VNum: [%s%5d%s], RNum: [%5d], Idnum: [%5ld], Type: %s, SpecProc: %s\r\n",
  ...
  send_to_char(
      ch, "Weight: %d, Value: %d, Cost/day: %d, Timer: %d, Min level: %d\r\n",
...
static void do_stat_character(struct char_data *ch, struct char_data *k) {
  ...
  send_to_char(
      ch, "%s %s '%s'  IDNum: [%5ld], In room [%5d], Loadroom : [%5d]\r\n", buf,
  ...
  send_to_char(ch, "%s%s %s, Lev: [%s%2d%s], XP: [%s%7d%s], Align: [%4d]\r\n",
               ...
               IS_NPC(k) ? "" : buf2, CCYEL(ch, C_NRM), GET_LEVEL(k),
               CCNRM(ch, C_NRM), CCYEL(ch, C_NRM), GET_EXP(k), CCNRM(ch, C_NRM),
               GET_ALIGNMENT(k));

  if (!IS_NPC(k)) {
    char buf1[64], buf2[64];

    strftime(buf1, sizeof(buf1), "%a %b %d %Y",
             localtime(&(k->player.time.birth)));
    strftime(buf2, sizeof(buf2), "%a %b %d %Y",
             localtime(&(k->player.time.logon)));
    ...
    if (GET_LEVEL(k) >= LVL_BUILDER) {
      if (GET_OLC_ZONE(k) == AEDIT_PERMISSION)
        send_to_char(ch, ", OLC[%sAedit%s]", CCCYN(ch, C_NRM),
                     CCNRM(ch, C_NRM));
      else if (GET_OLC_ZONE(k) == HEDIT_PERMISSION)
        send_to_char(ch, ", OLC[%sHedit%s]", CCCYN(ch, C_NRM),
                     CCNRM(ch, C_NRM));
  ...
  send_to_char(ch,
               ...
               CCCYN(ch, C_NRM), GET_INT(k), CCNRM(ch, C_NRM), CCCYN(ch, C_NRM),
               GET_WIS(k), CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_DEX(k),
               CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_CON(k), CCNRM(ch, C_NRM),
               CCCYN(ch, C_NRM), GET_CHA(k), CCNRM(ch, C_NRM));

  send_to_char(ch,
               "Hit p.:[%s%d/%d+%d%s]  Mana p.:[%s%d/%d+%d%s]  Move "
               "p.:[%s%d/%d+%d%s]\r\n",
  ...
  send_to_char(ch, "Gold: [%9d], Bank: [%9d] (Total: %d), ", GET_GOLD(k),
               GET_BANK_GOLD(k), GET_GOLD(k) + GET_BANK_GOLD(k));

  if (!IS_NPC(k))
    send_to_char(ch, "Screen %s[%s%d%sx%s%d%s]%s\r\n", CCCYN(ch, C_NRM),
                 CCYEL(ch, C_NRM), GET_SCREEN_WIDTH(k), CCNRM(ch, C_NRM),
                 CCYEL(ch, C_NRM), GET_PAGE_LENGTH(k), CCCYN(ch, C_NRM),
                 CCNRM(ch, C_NRM));

  send_to_char(ch,
               "AC: [%d%+d/10], Hitroll: [%2d], Damroll: [%2d], Saving throws: "
               "[%d/%d/%d/%d/%d]\r\n",
               GET_AC(k), dex_app[GET_DEX(k)].defensive, k->points.hitroll,
               k->points.damroll, GET_SAVE(k, 0), GET_SAVE(k, 1),
  ...
  if (k->affected) {
    for (aff = k->affected; aff; aff = aff->next) {
      send_to_char(ch, "SPL: (%3dhr) %s%-21s%s ", aff->duration + 1,
      ...
      if (aff->bitvector[0] || aff->bitvector[1] || aff->bitvector[2] ||
          aff->bitvector[3]) {
        if (aff->modifier)
          send_to_char(ch, ", ");
  ...
  if (!IS_NPC(k) && (GET_LEVEL(k) >= LVL_IMMORT)) {
    if (POOFIN(k))
      ...
      send_to_char(ch,
                   "%sPOOFIN:  %s%s appears with an ear-splitting bang.%s\r\n",
  ...
  if (!*buf1) {
    ...
    if ((object = get_obj_in_equip_vis(ch, name, &number, ch->equipment)) !=
      ...
      do_stat_character(ch, victim);
                  ...
                  ch, name, &number, world[IN_ROOM(ch)].contents)) != NULL)
      do_stat_object(ch, object);
...
ACMD(do_snoop) {
  ...
  if (!*arg)
    ...
    mudlog(BRF, GET_LEVEL(ch), TRUE, "(GC) %s snoops %s", GET_NAME(ch),
           GET_NAME(victim));
...
ACMD(do_switch) {
  ...
  if (ch->desc->original)
    ...
    mudlog(CMP, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE,
           "(GC) %s Switched into: %s", GET_NAME(ch), GET_NAME(victim));
  ...
  if (*buf) {
    ...
    if ((vict = get_char_vis(ch, buf, &number, FIND_CHAR_ROOM)) != NULL) {
      ...
      extract_char(vict);
                    ...
                    ch, buf, &number, world[IN_ROOM(ch)].contents)) != NULL) {
...
ACMD(do_advance) {
  ...
  if (newlevel < GET_LEVEL(victim)) {
    ...
    send_to_char(victim, "You are momentarily enveloped by darkness!\r\nYou "
                         "feel somewhat diminished.\r\n");
  } else {
    act("$n makes some strange gestures. A strange feeling comes upon you,\r\n"
        "Like a giant hand, light comes down from above, grabbing your "
        "body,\r\n"
...
ACMD(do_restore) {
  ...
  if (!*buf)
    ...
    mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s restored all",
           GET_NAME(ch));
...
ACMD(do_gecho) {
  ...
  if (!*argument)
    ...
    mudlog(CMP, MAX(LVL_BUILDER, GET_INVIS_LEV(ch)), TRUE,
           "(GC) %s gechoed: %s", GET_NAME(ch), argument);
...
ACMD(do_dc) {
  ...
  if (STATE(d) == CON_DISCONNECT || STATE(d) == CON_CLOSE)
    act("$E's already being disconnected.", FALSE, ch, 0, d->character,
        TO_CHAR);
  else {
...
static const char *last_array[11] = {
    "Connect",    "Enter Game", "Reconnect", "Takeover", "Quit",   "Idleout",
...
static void list_llog_entries(struct char_data *ch) {
  ...
  while (fread(&llast, sizeof(struct last_entry), 1, fp) == 1) {
    strftime(timestr, sizeof(timestr), "%a %b %d %Y %H:%M:%S",
             localtime(&llast.time));
    send_to_char(ch, "%10s    %d    %s    %s\r\n", llast.username,
                 llast.punique, last_array[llast.close_type], timestr);
    break;
  }
...
ACMD(do_last) {
  ...
  *name = '\0';
  ...
  if (*name && !num) {
    ...
    send_to_char(
        ch, "[%5ld] [%2d %s %s] %-12s : %-18s : %-24s\r\n", GET_IDNUM(vict),
        (int)GET_LEVEL(vict), race_abbrevs[(int)GET_RACE(vict)],
  ...
  while (num > 0 && recs > 0) {
    ...
    if (!*name || (*name && !str_cmp(name, mlast.username))) {
      strftime(timestr, sizeof(timestr), "%a %b %d %Y %H:%M",
               localtime(&mlast.time));
      send_to_char(ch, "%10.10s %20.20s %20.21s - ", mlast.username,
                   mlast.hostname, timestr);
...
ACMD(do_force) {
  ...
  if (!*arg || !*to_force)
    ...
    mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE,
           "(GC) %s forced room %d to %s", GET_NAME(ch),
...
ACMD(do_zreset) {
  ...
  if (i <= top_of_zone_table &&
      (can_edit_zone(ch, i) || GET_LEVEL(ch) > LVL_IMMORT)) {
    reset_zone(i);
    send_to_char(ch, "Reset zone #%d: %s.\r\n", zone_table[i].number,
                 zone_table[i].name);
    mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE,
           "(GC) %s reset zone %d (%s)", GET_NAME(ch), zone_table[i].number,
           zone_table[i].name);
  } else
    send_to_char(ch,
...
ACMD(do_wizutil) {
  ...
  if (!*arg)
    ...
    case SCMD_MUTE:
      ...
      mudlog(BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE,
             "(GC) Mute %s for %s by %s.", ONOFF(result), GET_NAME(vict),
             GET_NAME(ch));
...
static size_t print_zone_to_buf(char *bufptr, size_t left, zone_rnum zone,
  ...
  if (listall) {
    ...
    tmp = snprintf(bufptr, left,
                   "%3d %-30.30s%s By: %-10.10s%s Age: %3d; Reset: %3d (%s); "
                   "Range: %5d-%5d\r\n",
    ...
    for (i = 0; i < top_of_world; i++)
      if (world[i].number >= zone_table[zone].bot &&
    ...
    for (i = 0; i <= top_shop; i++)
      if (SHOP_NUM(i) >= zone_table[zone].bot &&
  ...
  return snprintf(bufptr, left, "%3d %-*s%s By: %-10.10s%s Range: %5d-%5d\r\n",
                  ...
                  count_color_chars(zone_table[zone].name) + 30,
...
ACMD(do_show) {
  ...
  struct show_struct {
    ...
    const char level;
                ...
                {"zones", LVL_IMMORT}, /* 1 */
                {"player", LVL_IMMORT}, {"rent", LVL_IMMORT},
                {"stats", LVL_IMMORT},  {"errors", LVL_IMMORT}, /* 5 */
                {"death", LVL_IMMORT},  {"godrooms", LVL_IMMORT},
                {"shops", LVL_IMMORT},  {"houses", LVL_IMMORT},
                {"snoop", LVL_IMMORT}, /* 10 */
                {"thaco", LVL_IMMORT},  {"exp", LVL_IMMORT},
                {"colour", LVL_IMMORT}, {"\n", 0}};
  ...
  case 2: {
    ...
    send_to_char(
        ch, "Gold: %-8d  Bal: %-8d Exp: %-8d  Align: %-5d  Lessons: %-3d\r\n",
  ...
  case 4:
    ...
    send_to_char(ch,
                 ...
                 "  %5d lists\r\n",
                 i, con, top_of_p_table + 1, j, top_of_mobt + 1, k,
                 top_of_objt + 1, top_of_world + 1, top_of_zone_table + 1,
                 top_of_trigt + 1, top_shop + 1, buf_largecount, total_quests,
    ...
    for (i = 0, k = 0; i <= top_of_world; i++)
      for (j = 0; j < DIR_COUNT; j++) {
        ...
        if (W_EXIT(i, j)->to_room == NOWHERE &&
            !W_EXIT(i, j)->general_description) {
          nlen =
              snprintf(buf + len, sizeof(buf) - len,
                       "%2d: (Nowhere) [%5d] %-*s%s (%s)\r\n", ++k,
                       GET_ROOM_VNUM(i), count_color_chars(world[i].name) + 40,
  ...
  case 11:
    ...
    for (j = 1; j < LVL_IMMORT; j++) {
      nlen = snprintf(buf + len, sizeof(buf) - len,
                      "%-3d - %-2d %-2d %-2d %-2d\r\n", j,
                      thaco(CLASS_MAGIC_USER, j), thaco(CLASS_CLERIC, j),
  ...
  case 12:
    ...
    for (i = 1; i < LVL_IMMORT; i++) {
      nlen = snprintf(
          buf + len, sizeof(buf) - len, "%-3d - %-6d %-6d %-6d %-6d\r\n", i,
    ...
    for (r = 0; r < 6; r++)
      for (g = 0; g < 6; g++)
        for (b = 0; b < 6; b++) {
          ...
          nlen = snprintf(buf + len, sizeof(buf) - len, "%s%s%s",
...
static int perform_set(struct char_data *ch, struct char_data *vict, int mode,
  ...
  case 37: /* olc */
    if (is_abbrev(val_arg, "socials") || is_abbrev(val_arg, "actions") ||
      ...
      send_to_char(
          ch,
          "Value must be a zone number, 'aedit', 'hedit', 'off' or 'all'.\r\n");
      return (0);
    } else
...
static void show_set_help(struct char_data *ch) {
  ...
  for (i = 0; *(set_fields[i].cmd) != '\n'; i++) {
    if (set_fields[i].level <= GET_LEVEL(ch)) {
      add_len = snprintf(buf + len, sizeof(buf) - len,
                         "%-20s%-5s  %-4s  %-6s\r\n", set_fields[i].cmd,
 ...
 * Structs.h. To ignore an apply, set max_aff to -99. These will be ignored if
 * MAX_APPLIES_LIMIT = 0 */
...
#define MIN_ROOM_DESC_LENGTH                                                   \
  80                         /* at least one line - set to 0 to not care.      \
                              */
...
ACMD(do_zcheck) {
  ...
  /*check mobs first*/
  for (i = 0; i < top_of_mobt; i++) {
    if (real_zone_by_thing(mob_index[i].vnum) ==
        zrnum) { /*is mob in this zone?*/
      mob = &mob_proto[i];
      ...
      if (!strncmp(mob->player.long_descr, "An unfinished mob stands here.",
                   30) &&
          (found = 1))
        len += snprintf(buf + len, sizeof(buf) - len,
      ...
      if (mob->player.description && *mob->player.description) {
        if (!strncmp(mob->player.description, "It looks unfinished.", 20) &&
            (found = 1))
          len += snprintf(buf + len, sizeof(buf) - len,
      ...
      if (mob->mob_specials.damsizedice == 1 &&
          mob->mob_specials.damnodice == 1 && GET_LEVEL(mob) == 0 &&
          (found = 1))
        len += snprintf(buf + len, sizeof(buf) - len,
      ...
      if (GET_EXP(mob) > MAX_EXP_ALLOWED && (found = 1))
        len += snprintf(buf + len, sizeof(buf) - len,
                        ...
                        MAX_EXP_ALLOWED);
      if ((AFF_FLAGGED(mob, AFF_CHARM) || AFF_FLAGGED(mob, AFF_POISON)) &&
          (found = 1))
        len += snprintf(buf + len, sizeof(buf) - len,
  ...
  for (i = 0; i < top_of_objt; i++) {
    if (real_zone_by_thing(obj_index[i].vnum) ==
        zrnum) { /*is object in this zone?*/
      obj = &obj_proto[i];
      ...
      if (!CAN_WEAR(obj, ITEM_WEAR_TAKE)) {
        if ((GET_OBJ_COST(obj) ||
            ...
            (found = 1))
          len += snprintf(buf + len, sizeof(buf) - len,
                          "- is NO_TAKE, but has cost (%d) weight (%d) or rent "
                          "(%d) set.\r\n",
  ...
  for (i = 0; i < top_of_world; i++) {
    if (world[i].zone == zrnum) {
      ...
      if ((MIN_ROOM_DESC_LENGTH) &&
        ...
        len +=
            snprintf(buf + len, sizeof(buf) - len,
                     "- Room description is too short. (%4.4d of min. %d "
                     "characters).\r\n",
...
static void trg_checkload(struct char_data *ch, trig_vnum tvnum) {
  ...
  send_to_char(ch, "Checking load info for the %s trigger '%s':\r\n",
               trig_index[trnum]->proto->attach_type == MOB_TRIGGER
                   ? "mobile"
                   : (trig_index[trnum]->proto->attach_type == OBJ_TRIGGER
                          ? "object"
                          : "room"),
...
ACMD(do_copyover) {
  ...
  for (d = descriptor_list; d; d = d_next) {
    ...
    if (!d->character || d->connected > CON_PLAYING) {
      ...
      fprintf(fp, "%d %ld %s %s %s\n", d->descriptor, GET_PREF(och),
              GET_NAME(och), d->host, CopyoverGet(d));
      /* save och */
...
ACMD(do_zpurge) {
  ...
  if (!purge_all) {
    ...
    mudlog(NRM, MAX(LVL_GRGOD, GET_INVIS_LEV(ch)), TRUE,
           "(GC) %s purged zone %d (%s)", GET_NAME(ch), zone_table[zone].number,
           zone_table[zone].name);
  } else {
...
ACMD(do_file) {
  ...
  if (GET_LEVEL(ch) < fields[l].level) {
    send_to_char(ch,
                 "You have not achieved a high enough level to view '%s'.\r\n",
                 fields[l].cmd);
    return;
  }
  ...
  snprintf(buf, sizeof(buf),
           "\tgFile:\tn %s\tg; Min. Level to read:\tn %d\tg; File Location:\tn "
           "%s\tg\r\n"
...
ACMD(do_changelog) {
  ...
  if (!(new = fopen(CHANGE_LOG_FILE, "w"))) {
    mudlog(BRF, LVL_IMPL, TRUE, "SYSERR: Error opening new changelog file (%s)",
           CHANGE_LOG_FILE);
    return;
  }
...
ACMD(do_plist) {
  ...
  int low = 0, high = LVL_IMPL, low_day = 0, high_day = 10000, low_hr = 0,
      high_hr = 24;
...
bool change_player_name(struct char_data *ch, struct char_data *vict,
                        char *new_name) {
  ...
  if (player_table[i].id != GET_IDNUM(vict)) {
    ...
    log("SYSERR: Player %s, with ID %ld, could not be found in the player "
        "index.",
  ...
  for (k = 0;
       (*(player_table[i].name + k) = LOWER(*(player_table[i].name + k))); k++)
    ;

  free(GET_PC_NAME(vict));
  GET_PC_NAME(vict) =
      strdup(CAP(new_name)); // Change the name in the victims char struct
  ...
  mudlog(BRF, LVL_IMMORT, TRUE, "(GC) %s changed the name of %s to %s",
...
ACMD(do_zlock) {
  ...
  if (!*arg) {
    ...
    send_to_char(
        ch, "Locks a zone so that building or editing is not possible.\r\n");
    send_to_char(ch, "The 'list' shows all currently locked zones.\r\n");
    send_to_char(
        ch, "'zlock all' will lock every zone with the GRID flag set.\r\n");
  ...
  if (is_abbrev(arg, "all")) {
    if (GET_LEVEL(ch) < LVL_GRGOD) {
      send_to_char(ch,
                   "You do not have sufficient access to lock all zones.\r\n");
      return;
    }
    if (!*arg2) {
      for (zn = 0; zn <= top_of_zone_table; zn++) {
        if (!ZONE_FLAGGED(zn, ZONE_NOBUILD) && ZONE_FLAGGED(zn, ZONE_GRID)) {
          ...
          if (save_zone(zn)) {
            log("(GC) %s has locked zone %d", GET_NAME(ch),
    ...
    mudlog(BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE,
           "(GC) %s has locked ALL zones!", GET_NAME(ch));
    return;
  }
  if (is_abbrev(arg, "list")) {
    ...
    for (zn = 0; zn <= top_of_zone_table; zn++) {
      if (ZONE_FLAGGED(zn, ZONE_NOBUILD)) {
        ...
        send_to_char(
            ch, "[%s%3d%s] %s%-*s %s%-1s%s\r\n", QGRN, zone_table[zn].number,
            QNRM, QCYN, count_color_chars(zone_table[zn].name) + 30,
  ...
  if (GET_LEVEL(ch) < LVL_GRGOD &&
      !is_name(GET_NAME(ch), zone_table[zn].builders) &&
      GET_OLC_ZONE(ch) != znvnum) {
    send_to_char(ch,
                 "You do not have sufficient access to lock that zone!\r\n");
    return;
  }
  ...
  if (save_zone(zn)) {
    mudlog(NRM, MAX(LVL_GRGOD, GET_INVIS_LEV(ch)), TRUE,
           "(GC) %s has locked zone %d", GET_NAME(ch), znvnum);
  } else {
    send_to_char(ch, "Unable to save zone changes.  Check syslog!\r\n");
  }
}
...
ACMD(do_zunlock) {
  ...
  if (is_abbrev(arg, "all")) {
    ...
    for (zn = 0; zn <= top_of_zone_table; zn++) {
      if (ZONE_FLAGGED(zn, ZONE_NOBUILD)) {
        ...
        if (save_zone(zn)) {
          log("(GC) %s has unlocked zone %d", GET_NAME(ch),
              zone_table[zn].number);
        } else {
          fail = TRUE;
        }
  ...
  if (GET_LEVEL(ch) < LVL_GRGOD &&
      ...
      GET_OLC_ZONE(ch) != znvnum) {
    send_to_char(ch,
                 "You do not have sufficient access to unlock that zone!\r\n");
    return;
  }
...
ACMD(do_recent) {
  ...
  if (GET_LEVEL(ch) >= LVL_GRGOD) { /* If High-Level Imm, then show Host IP */
    send_to_char(ch, " ID | DATE/TIME                | HOST IP                 "
                     "         | Player Name\r\n");
  } else {
...
ACMD(do_oset) {
  ...
  if (!*arg)
    send_to_char(ch, usage);
           ...
           !(obj = get_obj_in_list_vis(ch, arg, NULL,
    ...
    if (!*arg2)
      ...
      else if (is_abbrev(arg2, "apply") &&
               (success = oset_apply(obj, argument)))
