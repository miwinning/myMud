...
static void perform_complex_alias(struct txt_q *input_q, char *orig,
                                  struct alias_data *a);
...
cpp_extern const struct command_info cmd_info[] = {
    ...
    {"buildwalk", "buildwalk", POS_STANDING, do_gen_tog, LVL_BUILDER,
     SCMD_BUILDWALK},
    ...
    {"roomflags", "roomflags", POS_DEAD, do_gen_tog, LVL_IMMORT,
     SCMD_SHOWVNUMS},
    ...
    {"slist", "slist", POS_SLEEPING, do_oasis_list, LVL_BUILDER,
     SCMD_OASIS_SLIST},
...
static const char *fill[] = {"in", "from", "with", "the",
                             "on", "at",   "to",   "\n"};

static const char *reserved[] = {"a",    "an",      "self",      "me", "all",
  ...
  for (k = descriptor_list; k; k = next_k) {
    ...
    if (!strcmp(GET_NAME(k->character), GET_NAME(d->character))) {
      ...
      if ((STATE(k) > CON_PLAYING) && (STATE(k) < CON_QCLASS)) {
        ...
        d->character = NULL;
        d->original = NULL;
        write_to_output(d, "\r\nSorry, due to multiple connections, all your "
                           "connections are being closed.\r\n");
        write_to_output(d, "\r\nPlease reconnect.\r\n");
        STATE(d) = CON_CLOSE;

        mudlog(NRM, LVL_GOD, TRUE,
               "SYSERR: Multiple logins with 1st in-game and the 2nd in char "
               "creation.");
...
EVENTFUNC(get_protocols) {
  ...
  if (d->pProtocol->pVariables[eMSDP_XTERM_256_COLORS]->ValueInt)
    len += snprintf(buf + len, MAX_STRING_LENGTH - len,
                    "\tO[\toColors\tO] \tw256\tn | ");
...
void nanny(struct descriptor_data *d, char *arg) {
  ...
  struct {
    ...
    void (*func)(struct descriptor_data *, char *);
      ...
      {CON_OEDIT, oedit_parse},       {CON_ZEDIT, zedit_parse},
      ...
      {CON_MSGEDIT, msgedit_parse},   {-1, NULL}};
  ...
  case CON_PASSWORD: /* get pwd for known player      */
     ...
     * reloaded from disk at this point (after the password has been typed).
     * However I'm afraid that trying to load a character over an already loaded
     * character is going to cause some problem down the road that I can't see
     * at the moment. So to compensate, I'm going to (1) add a 15 or 20-second
     * time limit for entering a password, and (2) re-add the code to cut off
     * duplicates when a player quits.  JE 6 Feb 96 */
    ...
    if (!*arg)
      ...
      if (strncmp(CRYPT(arg, GET_PASSWD(d->character)),
        ...
        mudlog(BRF, LVL_GOD, TRUE, "Bad PW: %s [%s]", GET_NAME(d->character),
               d->host);
        GET_BAD_PWS(d->character)++;
        save_char(d->character);
        if (++(d->bad_pws) >=
            CONFIG_MAX_BAD_PWS) { /* 3 strikes and you're out. */
      ...
      if (isbanned(d->host) == BAN_SELECT &&
        ...
        write_to_output(d, "Sorry, this char has not been cleared for login "
                           "from your site!\r\n");
        STATE(d) = CON_CLOSE;
      ...
      if (GET_LEVEL(d->character) < circle_restrict) {
        write_to_output(
            d, "The game is temporarily restricted.. try again later.\r\n");
        STATE(d) = CON_CLOSE;
        mudlog(NRM, LVL_GOD, TRUE,
      ...
      if (GET_INVIS_LEV(d->character))
        ...
        mudlog(BRF, LVL_IMMORT, TRUE, "%s has connected.",
               GET_NAME(d->character));

      /* Add to the list of 'recent' players (since last reboot) */
      if (AddRecentPlayer(GET_NAME(d->character), d->host, FALSE, FALSE) ==
          FALSE) {
        mudlog(BRF, MAX(LVL_IMMORT, GET_INVIS_LEV(d->character)), TRUE,
  ...
  case CON_QCLASS:
    ...
    if (load_result == CLASS_UNDEFINED ||
        !classRaceAllowed[GET_RACE(d->character)][load_result]) {
      write_to_output(d, "\r\nThat's not a class.\r\nClass: ");
      return;
    }
    ...
    mudlog(NRM, LVL_GOD, TRUE, "%s [%s] new player.", GET_NAME(d->character),
           d->host);

    /* Add to the list of 'recent' players (since last reboot) */
    if (AddRecentPlayer(GET_NAME(d->character), d->host, TRUE, FALSE) ==
        FALSE) {
      mudlog(BRF, MAX(LVL_IMMORT, GET_INVIS_LEV(d->character)), TRUE,
             "Failure to AddRecentPlayer (returned FALSE).");
    }
  ...
  case CON_MENU: { /* get selection from main menu  */
    ...
    case '2':
      if (d->character->player.description) {
         ...
         * current buffer in the editor.  Do setup the ABORT buffer here,
         * however. */
