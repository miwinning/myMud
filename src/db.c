...
static void interpret_espec(const char *keyword, const char *value, int i,
                            int nr);
...
char *fread_action(FILE *fl, int nr) {
  ...
  if (fgets(buf, MAX_STRING_LENGTH, fl) == NULL) {
    if (feof(fl)) {
      ...
      log("SYSERR: fread_action: read error near action #%d: %s", nr,
          strerror(errno));
    }
    exit(1);
  }
...
static void boot_social_messages(void) {
  ...
  if (CONFIG_NEW_SOCIALS == TRUE) {
    ...
    *next_soc = '\0';
    ...
    if (ferror(fl)) {
      log("SYSERR: error encountered reading socials file %s: %s",
          SOCMESS_FILE_NEW, strerror(errno));
      exit(1);
    }
  } else { /* old style */
  ...
  for (line_number = 0;; ++line_number) {
    if (fscanf(fl, " %s ", next_soc) != 1) {
      if (feof(fl))
        log("SYSERR: unexpected end of file encountered in socials file %s",
            SOCMESS_FILE_NEW);
      else if (ferror(fl))
        log("SYSERR: error reading socials file %s: %s", SOCMESS_FILE_NEW,
            strerror(errno));
      else
    ...
    if (CONFIG_NEW_SOCIALS == TRUE) {
      if (fscanf(fl, " %s %d %d %d %d \n", sorted, &hide, &min_char_pos,
         ...
         * the server is expecting to find the remainder of the first line of
         * the social ('hide' and 'minimum position').  These must follow the
         * name of the social with a single space such as: 'accuse 0 5\n'. This
         * error often occurs when one of the numbers is missing or the social
...
void free_text_files(void) {
  char **textfiles[] = {&wizlist,  &immlist,    &news,      &credits, &motd,
                        &imotd,    &help,       &ihelp,     &info,    &policies,
                        &handbook, &background, &GREETINGS, &bugs,    &typos,
                        &ideas,    NULL};
  int rf;
...
void destroy_db(void) {
  ...
  for (cnt = 0; cnt <= top_of_world; cnt++) {
    ...
    for (itr = 0; itr < NUM_OF_DIRS;
         itr++) { /* NUM_OF_DIRS here, not DIR_COUNT */
...
static void reset_time(void) {
  ...
  if ((bgtime = fopen(TIME_FILE, "r")) == NULL)
    ...
    if (fscanf(bgtime, "%ld\n", (long *)&beginning_of_time) == EOF) {
      if (feof(bgtime)) {
        log("SYSERR: reset_time: unexpected end of file encountered reading "
            "%s.",
            TIME_FILE);
...
void index_boot(int mode) {
  ...
  for (line_number = 0;; ++line_number) {
    ...
    if (fscanf(db_index, "%s\n", buf1) != 1) {
      if (feof(db_index))
        log("SYSERR: boot error -- unexpected end of file encountered in index "
            "file ./%s%s. "
  ...
  case DB_BOOT_MOB:
    ...
    log("   %d mobs, %d bytes in index, %d bytes in prototypes.", rec_count,
        size[0], size[1]);
    break;
  case DB_BOOT_OBJ:
    ...
    log("   %d objs, %d bytes in index, %d bytes in prototypes.", rec_count,
        size[0], size[1]);
    break;
  case DB_BOOT_ZON:
...
void discrete_load(FILE *fl, int mode, char *filename) {
  ...
  const char *modes[] = {"world", "mob", "obj", "ZON",
                         "SHP",   "HLP", "trg", "qst"};
...
void parse_room(FILE *fl, int virtual_nr) {
  ...
  if (virtual_nr < zone_table[zone].bot) {
    log("SYSERR: Room #%d is below zone %d (bot=%d, top=%d).", virtual_nr,
  ...
  if (((retval = sscanf(line, " %d %s %s %s %s %d ", t, flags, flags2, flags3,
    ...
    check_bitvector_names(world[room_nr].room_flags[0], room_bits_count, flags,
                          "room");
    ...
    for (taeller = 0; taeller < AF_ARRAY_MAX; taeller++)
      check_bitvector_names(world[room_nr].room_flags[taeller], room_bits_count,
                            flags, "room");
...
void setup_dir(FILE *fl, int room, int dir) {
  ...
  snprintf(buf2, sizeof(buf2), "room #%d, direction D%d",
           GET_ROOM_VNUM(room) + 1, dir);
  ...
  world[room].dir_option[dir]->key =
      ((t[1] == -1 || t[1] == 65535) ? NOTHING : t[1]);
  world[room].dir_option[dir]->to_room =
      ((t[2] == -1 || t[2] == 0) ? NOWHERE : t[2]);
}
...
static void interpret_espec(const char *keyword, const char *value, int i,
  ...
  CASE("SavingBreath") {
    RANGE(0, 100);
    mob_proto[i].char_specials.saved.apply_saving_throw[SAVING_BREATH] =
        num_arg;
  }
...
void parse_mobile(FILE *mob_f, int nr) {
  ...
  if (((retval = sscanf(line, "%s %s %s %s %s %s %s %s %d %c", f1, f2, f3, f4,
    ...
    check_bitvector_names(MOB_FLAGS(mob_proto + i)[0], action_bits_count, buf2,
                          "mobile");
    ...
    check_bitvector_names(AFF_FLAGS(mob_proto + i)[0], affected_bits_count,
                          buf2, "mobile affect");
    ...
    for (taeller = 0; taeller < AF_ARRAY_MAX; taeller++)
      check_bitvector_names(MOB_FLAGS(mob_proto + i)[taeller],
                            action_bits_count, buf2, "mobile");
    ...
    for (taeller = 0; taeller < AF_ARRAY_MAX; taeller++)
      check_bitvector_names(AFF_FLAGS(mob_proto + i)[taeller],
                            affected_bits_count, buf2, "mobile affect");
  } else {
    log("SYSERR: Format error after string section of mob #%d\n ...expecting "
        "line of form '# # # {S | E}'",
        nr);
...
char *parse_object(FILE *obj_f, int nr) {
  ...
  int t[10], j, retval;
  char *tmpptr, buf2[128], f1[READ_SIZE], f2[READ_SIZE], f3[READ_SIZE],
      f4[READ_SIZE];
  ...
  if ((retval = sscanf(line, "%d %d %d %d %d", t, t + 1, t + 2, t + 3,
                       t + 4)) != 5) {
    if (retval == 3) {
      t[3] = 0;
      t[4] = 0;
      ...
      log("SYSERR: Format error in third numeric line (expecting 5 args, got "
          "%d), %s",
          retval, buf2);
      exit(1);
    }
...
static void load_zones(FILE *fl, char *zonename) {
  ...
  if (sscanf(buf, " %hd %hd %d %d %s %s %s %s %d %d", &Z.bot, &Z.top,
    ...
    if (sscanf(buf, " %hd %hd %d %d ", &Z.bot, &Z.top, &Z.lifespan,
               &Z.reset_mode) != 4) {
...
struct char_data *read_mobile(mob_vnum nr, int type) /* and mob_rnum */
{
  ...
  *mob = mob_proto[i];
  ...
  if (!mob->points.max_hit) {
    mob->points.max_hit =
        dice(mob->points.hit, mob->points.mana) + mob->points.move;
  } else
...
struct obj_data *read_object(obj_vnum nr, int type) /* and obj_rnum */
{
  ...
  if (i == NOTHING || i > top_of_objt) {
    log("Object (%c) %d does not exist in database.",
        type == VIRTUAL ? 'V' : 'R', nr);
    return (NULL);
  }
    ...
    if (zone_table[update_u->zone_to_reset].reset_mode == 2 ||
      ...
      for (pt = descriptor_list; pt; pt = pt->next)
        if (IS_PLAYING(pt) && pt->character &&
            PRF_FLAGGED(pt->character, PRF_ZONERESETS))
      ...
      if (ZCMD.arg1 == MOB_TRIGGER && tmob) {
        ...
        if (ZCMD.arg3 == NOWHERE || ZCMD.arg3 > top_of_world) {
          ...
          if (!(world[ZCMD.arg3].script)) {
            ...
            add_var(&(world[ZCMD.arg3].script->global_vars), ZCMD.sarg1,
                    ZCMD.sarg2, ZCMD.arg2);
          last_cmd = 1;
        }
      }
      break;
...
/* Read to end of line from a given file and convert to flag values, then return
 * number of ints */
...
static int file_to_string_alloc(const char *name, char **buf) {
  ...
  for (in_use = descriptor_list; in_use; in_use = in_use->next) {
    ...
    paginate_string((in_use->showstr_head = strdup(*in_use->showstr_vector)),
                    in_use);
...
static int check_object(struct obj_data *obj) {
  ...
  for (y = 0; y < TW_ARRAY_MAX; y++) {
    error |= check_bitvector_names(GET_OBJ_WEAR(obj)[y], wear_bits_count,
                                   objname, "object wear");
    error |= check_bitvector_names(GET_OBJ_EXTRA(obj)[y], extra_bits_count,
                                   objname, "object extra");
    error |= check_bitvector_names(GET_OBJ_AFFECT(obj)[y], affected_bits_count,
                                   objname, "object affect");
  }
  ...
  case ITEM_DRINKCON: {
    ...
    if (search_block(onealias, drinknames, TRUE) < 0 && (error = TRUE))
      log("SYSERR: Object #%d (%s) doesn't have drink type as last keyword. "
          "(%s)",
    ...
    if (obj->ex_description) {
      ...
      do {
        if (find_exdesc(onealias, obj->ex_description) && (error = TRUE)) {
          log("SYSERR: Object #%d (%s) is type NOTE and has extra description "
              "with same name. (%s)",
              GET_OBJ_VNUM(obj), obj->short_description, obj->name);
        }
...
static int check_bitvector_names(bitvector_t bits, size_t namecount,
  ...
  for (flagnum = namecount; flagnum < sizeof(bitvector_t) * 8; flagnum++)
    if ((1 << flagnum) & bits) {
      log("SYSERR: %s has unknown %s flag, bit %d (0 through %d known).",
          whatami, whatbits, flagnum, (int)namecount - 1);
      error = TRUE;
    }
