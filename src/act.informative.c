...
static void show_obj_to_char(struct obj_data *obj, struct char_data *ch,
  ...
  if (!obj || !ch) {
    log("SYSERR: NULL pointer in show_obj_to_char(): obj=%p ch=%p", (void *)obj,
        (void *)ch);
  ...
  if ((mode == 0) && obj->description) {
    if (GET_OBJ_VAL(obj, 1) != 0 || OBJ_SAT_IN_BY(obj)) {
      ...
      if (found) {
        send_to_char(ch, "You are %s upon %s.",
...
static void list_one_char(struct char_data *i, struct char_data *ch) {
  ...
  const char *positions[] = {" is lying here, dead.",
...
static void list_char_to_char(struct char_data *list, struct char_data *ch) {
  ...
  for (i = list; i; i = i->next_in_room)
    if (ch != i) {
      /* hide npcs whose description starts with a '.' from non-holylighted
       * people - Idea from Elaseth of TBA */
...
ACMD(do_look) {
  ...
  if (GET_POS(ch) < POS_SLEEPING)
    ...
    /* did the char type 'look <direction>?' */
      ...
      look_at_target(ch, strcpy(tempsave, arg), 0);
  }
}
...
ACMD(do_score) {
  ...
  send_to_char(
      ch, "You have %d(%d) hit, %d(%d) mana and %d(%d) movement points.\r\n",
  ...
  if (GET_QUEST(ch) == NOTHING)
    ...
    send_to_char(ch, "and your current quest is: %s",
...
#define WHO_FORMAT                                                             \
  "Usage: who [minlev[-maxlev]] [-n name] [-d racelist] [-c classlist] [-k] "  \
  "[-l] [-n] [-q] [-r] [-s] [-z]\r\n"
...
ACMD(do_who) {
  ...
  for (d = descriptor_list; d && !short_list; d = d->next) {
    ...
    if (CAN_SEE(ch, tch) && IS_PLAYING(d)) {
      ...
      if (showleader && (!GROUP(tch) || GROUP_LEADER(GROUP(tch)) != tch))
        continue;
      for (i = 0; *rank[i].disp != '\n'; i++)
        if (GET_LEVEL(tch) >= rank[i].min_level &&
            GET_LEVEL(tch) <= rank[i].max_level)
          rank[i].count++;
    }
  }
    ...
    for (d = descriptor_list; d; d = d->next) {
      ...
      if (short_list) {
        ...
        if (*get_clan_name(tch)) {
          send_to_char(ch, " %s[%s]%s", CCYEL(ch, C_SPR), get_clan_name(tch),
                       CCNRM(ch, C_SPR));
        }
  ...
  if (IS_HAPPYHOUR > 0) {
    send_to_char(ch, "It's a Happy Hour! Type \tRhappyhour\tW to see the "
                     "current bonuses.\r\n");
  }
}

#define USERS_FORMAT                                                           \
  "format: users [-l minlevel[-maxlevel]] [-n name] [-h host] [-r racelist] "  \
  "[-c classlist] [-o] [-p]\r\n"
  ...
  if (!*arg) {
        ...
        if (i && CAN_SEE(ch, i) && (IN_ROOM(i) != NOWHERE)) {
          if (d->original)
            send_to_char(ch, "%-8s%s - [%5d] %s%s (in %s%s)\r\n", GET_NAME(i),
                         QNRM, GET_ROOM_VNUM(IN_ROOM(d->character)),
    ...
    for (i = character_list; i; i = i->next)
      if (CAN_SEE(ch, i) && IN_ROOM(i) != NOWHERE &&
        ...
        found = 1;
        send_to_char(ch, "M%3d. %-25s%s - [%5d] %-25s%s", ++num, GET_NAME(i),
                     QNRM, GET_ROOM_VNUM(IN_ROOM(i)), world[IN_ROOM(i)].name,
                     QNRM);
        if (SCRIPT(i) && TRIGGERS(SCRIPT(i))) {
          if (!TRIGGERS(SCRIPT(i))->next)
            send_to_char(ch, "[T%d] ", GET_TRIG_VNUM(TRIGGERS(SCRIPT(i))));
  ...
  if (*arg) {
    if (isdigit(*arg)) {
      ...
      send_to_char(ch, "%slevels       %s- shows all levels (1-%d)\r\n", QCYN,
                   QNRM, (LVL_IMMORT - 1));
      send_to_char(ch,
                   "%slevels 5     %s- shows 5 levels either side of your "
                   "current level\r\n",
                   QCYN, QNRM);
...
ACMD(do_toggle) {
  ...
  const struct {
    ...
    char *enable_msg;
      ...
      {"slownameserver", 0, LVL_IMPL,
       "Nameserver_is_slow changed to OFF; IP addresses will now be "
       "resolved.\r\n",
       "Nameserver_is_slow changed to ON; sitenames will no longer be "
       "resolved.\r\n"},
      ...
      {"autokey", PRF_AUTOKEY, 0,
       ...
       "You will now automatically unlock doors when opening them (if you have "
       "the key).\r\n"},
      {"autodoor", PRF_AUTODOOR, 0,
       "You will now need to specify a door direction when opening, closing "
       "and unlocking.\r\n",
       "You will now find the next available door when opening, closing or "
       "unlocking.\r\n"},
  ...
  if (!*arg) {
    ...
    send_to_char(
        ...
        ONOFF(PRF_FLAGGED(ch, PRF_NOSHOUT)), buf2,
  ...
  case SCMD_WIMPY:
    ...
    if (isdigit(*arg2)) {
      ...
      send_to_char(ch, "Specify at how many hit points you want to wimp out "
                       "at.  (0 to disable)\r\n");
    break;
  ...
  if (!(GET_LEVEL(victim) < LVL_IMMORT) ||
    ...
    if (!got_from_file) {
      ...
      if (PRF_FLAGGED(victim, PRF_AFK))
        send_to_char(ch,
                     "%s%s is afk right now, so %s may not respond to "
                     "communication.%s\r\n",
                     CBGRN(ch, C_NRM), GET_NAME(victim), HSSH(victim),
                     CCNRM(ch, C_NRM));
    } else if (hours > 0)
...
static bool get_zone_levels(zone_rnum znum, char *buf) {
  ...
  sprintf(buf, "Levels %d to %d", zone_table[znum].min_level,
...
ACMD(do_areas) {
  ...
  for (i = 0; i <= top_of_zone_table;
       i++) { /* Go through the whole zone table */
    show_zone = FALSE;
    overlap = FALSE;
    ...
    if (ZONE_FLAGGED(i, ZONE_GRID)) { /* Is this zone 'on the grid' ?    */
      if (lolev == -1) {
        ...
        show_zone = TRUE;
      } else if ((hilev == -1) && (lolev >= ZONE_MINLVL(i)) &&
                 (lolev <= ZONE_MAXLVL(i))) {
    ...
    if (show_zone) {
      ...
      tmp_len = snprintf(
          buf + len, sizeof(buf) - len, "\tn(%3d) %s%-*s\tn %s%s\tn\r\n",
  ...
  if (overlap_shown) {
    snprintf(buf + len, sizeof(buf) - len,
             "Areas shown in \trred\tn may have some creatures outside the "
             "specified range.\r\n");
  }
...
static void list_scanned_chars(struct char_data *list, struct char_data *ch,
  ...
  *buf = '\0';
  ...
  for (i = list; i; i = i->next_in_room) {
    ...
    /* make sure to add changes to the if statement above to this one also,
       using or's to join them.. i.e., if (!CAN_SEE(ch, i) || !condition2 ||
       !condition3) */
      ...
      if (world[scanned_room].dir_option[door] &&
        ...
        if (IS_DARK(scanned_room) && !CAN_SEE_IN_DARK(ch)) {
          if (world[scanned_room].people)
            send_to_char(
                ch, "%s: It's too dark to see, but you can hear shuffling.\r\n",
                dirs[door]);
          else
