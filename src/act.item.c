...
static int perform_drop(struct char_data *ch, struct obj_data *obj, byte mode,
                        const char *sname, room_rnum RDR);
static void perform_drop_gold(struct char_data *ch, int amount, byte mode,
                              room_rnum RDR);
...
static void get_from_room(struct char_data *ch, char *arg, int howmany) {
  ...
  if (dotmode == FIND_INDIV) {
    if (!(obj = get_obj_in_list_vis(ch, arg, NULL,
...
ACMD(do_get) {
  ...
  if (!*arg1)
    ...
    if (cont_dotmode == FIND_INDIV) {
      mode = generic_find(arg2, FIND_OBJ_INV | FIND_OBJ_ROOM, ch, &tmp_char,
                          &cont);
      if (!cont)
...
static void perform_drop_gold(struct char_data *ch, int amount, byte mode,
  ...
  if (amount <= 0)
    ...
    if (mode != SCMD_JUNK) {
      ...
      if (mode == SCMD_DONATE) {
        send_to_char(ch, "You throw some gold into the air where it disappears "
                         "in a puff of smoke!\r\n");
      ...
      send_to_char(
          ch, "You drop some gold which disappears in a puff of smoke!\r\n");
    }
...
static void perform_give_gold(struct char_data *ch, struct char_data *vict,
  ...
  snprintf(buf, sizeof(buf), "$n gives you %d gold coin%s.", amount,
           amount == 1 ? "" : "s");
...
ACMD(do_drink) {
  ...
  if (subcmd == SCMD_DRINK) {
    ...
    if (drink_aff[GET_OBJ_VAL(temp, 2)][DRUNK] > 0)
      amount =
          (25 - GET_COND(ch, THIRST)) / drink_aff[GET_OBJ_VAL(temp, 2)][DRUNK];
    else
  ...
  gain_condition(ch, DRUNK,
                 drink_aff[GET_OBJ_VAL(temp, 2)][DRUNK] * amount / 4);
  gain_condition(ch, HUNGER,
                 drink_aff[GET_OBJ_VAL(temp, 2)][HUNGER] * amount / 4);
  ...
  if (GET_OBJ_VAL(temp, 3) &&
      GET_LEVEL(ch) < LVL_IMMORT) { /* The crap was poisoned ! */
...
static void perform_wear(struct char_data *ch, struct obj_data *obj,
                         int where) {
  /*
  ...
  int wear_bitvectors[] = {ITEM_WEAR_TAKE,  ITEM_WEAR_FINGER, ITEM_WEAR_FINGER,
...
ACMD(do_sac) {
  ...
  case 4:
    send_to_char(
        ch, "Your sacrifice to the Gods is rewarded with %d gold coins.\r\n",
        1 + GET_OBJ_LEVEL(j));
