...
obj_rnum adjust_objects(obj_rnum refpt) {
  ...
  for (obj = object_list; obj; obj = obj->next)
    GET_OBJ_RNUM(obj) +=
        (GET_OBJ_RNUM(obj) != NOTHING && GET_OBJ_RNUM(obj) >= refpt);
    ...
    if ((realcounter = real_object(counter)) != NOTHING) {
      ...
      if (n >= MAX_STRING_LENGTH) {
        mudlog(BRF, LVL_BUILDER, TRUE,
               "SYSERR: Could not save object #%d due to size (%d > maximum of "
               "%d).",
      ...
      fprintf(fp,
              ...
              GET_OBJ_TYPE(obj), ebuf1, ebuf2, ebuf3, ebuf4, wbuf1, wbuf2,
              wbuf3, wbuf4, pbuf1, pbuf2, pbuf3, pbuf4, GET_OBJ_VAL(obj, 0),
              GET_OBJ_VAL(obj, 1), GET_OBJ_VAL(obj, 2), GET_OBJ_VAL(obj, 3),
              GET_OBJ_WEIGHT(obj), GET_OBJ_COST(obj), GET_OBJ_RENT(obj),
              GET_OBJ_LEVEL(obj), GET_OBJ_TIMER(obj));
...
void free_object_strings_proto(struct obj_data *obj) {
  ...
  if (obj->description && obj->description != obj_proto[robj_num].description)
    free(obj->description);
  if (obj->short_description &&
      obj->short_description != obj_proto[robj_num].short_description)
    free(obj->short_description);
  if (obj->action_description &&
      obj->action_description != obj_proto[robj_num].action_description)
    free(obj->action_description);
  if (obj->ex_description) {
...
bool oset_apply(struct obj_data *obj, char *argument) {
  ...
  /* There is no slot already using our APPLY_XXX type, and no empty slots
   * either */
  if (location == -1)
    return FALSE;
  ...
  /* Our modifier is set at 0, so lets just clear the apply location so that it
   * may be reused at a later point */
