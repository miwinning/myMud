  ...
  if (str_cmp(obj_name, "coins") && str_cmp(obj_name, "gold")) {
    ...
    if (!(obj = get_obj_in_list_vis(ch, obj_name, NULL, vict->carrying))) {
      ...
      if (percent > GET_SKILL(ch, SKILL_STEAL)) {
        ...
        act("$n tried to steal something from you!", FALSE, ch, 0, vict,
            TO_VICT);
...
static void print_group(struct char_data *ch) {
  ...
  while ((k = (struct char_data *)simple_list(ch->group->members)) != NULL)
    send_to_char(ch, "%-*s: %s[%4d/%-4d]H [%4d/%-4d]M [%4d/%-4d]V%s\r\n",
                 ...
                 GROUP_LEADER(GROUP(ch)) == k ? CBGRN(ch, C_NRM)
                                              : CCGRN(ch, C_NRM),
                 GET_HIT(k), GET_MAX_HIT(k), GET_MANA(k), GET_MAX_MANA(k),
                 GET_MOVE(k), GET_MAX_MOVE(k), CCNRM(ch, C_NRM));
}
...
static void display_group_list(struct char_data *ch) {
    ...
    while ((group = (struct group_data *)simple_list(group_list)) != NULL) {
      ...
      if (GROUP_LEADER(group) && !IS_SET(GROUP_FLAGS(group), GROUP_ANON))
        send_to_char(ch, "%-2d) %s%-12s     %-2d              %s%s\r\n",
                     ++count,
                     IS_SET(GROUP_FLAGS(group), GROUP_OPEN) ? CCGRN(ch, C_NRM)
                                                            : CCRED(ch, C_NRM),
  ...
  if (count)
    send_to_char(ch,
                 ...
                 "%sSeeking Members%s\r\n"
                 "%sClosed%s\r\n",
                 CCGRN(ch, C_NRM), CCNRM(ch, C_NRM), CCRED(ch, C_NRM),
                 CCNRM(ch, C_NRM));
  else
    send_to_char(ch, "\r\n"
...
ACMD(do_group) {
  ...
  if (!*buf) {
    if (GROUP(ch))
      ...
      send_to_char(ch, "You must specify a group option, or type HELP GROUP "
                       "for more info.\r\n");
    return;
  }
  ...
  if (is_abbrev(buf, "new")) {
    ...
    if (is_abbrev(argument, "open")) {
      ...
      send_to_char(ch, "The group is now %s to new members.\r\n",
                   IS_SET(GROUP_FLAGS(GROUP(ch)), GROUP_OPEN) ? "open"
                                                              : "closed");
      ...
      send_to_char(ch, "The group location is now %s to other players.\r\n",
                   IS_SET(GROUP_FLAGS(GROUP(ch)), GROUP_ANON) ? "invisible"
                                                              : "visible");
    } else
      send_to_char(ch, "The flag options are: Open, Anonymous\r\n");
  } else {
    send_to_char(ch, "You must specify a group option, or type HELP GROUP for "
                     "more info.\r\n");
  }
}
...
ACMD(do_split) {
  ...
  if (is_number(buf)) {
    ...
    send_to_char(ch,
                 "You split %d coins among %d members -- %d coins each.\r\n",
                 amount, num, share);
...
ACMD(do_happyhour) {
  ...
  if (is_abbrev(arg, "experience")) {
    ...
    send_to_char(
        ch,
        "Happy Hour Time set to %d ticks (%d hours %d mins and %d secs)\r\n",
    ...
    send_to_char(
        ...
        "       \tyhappyhour default      \tw- sets a default setting for "
        "happyhour\r\n\r\n"
        ...
        "\tyhappyhour time\tn will therefore stop the happyhour timer.\r\n",
