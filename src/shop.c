...
static void push(struct stack_data *stack,
                 int pushval);            /**< @todo Move to utils.c */
...
static bool shopping_identify(char *arg, struct char_data *ch,
                              struct char_data *keeper, int shop_nr);
...
static int buy_price(struct obj_data *obj, int shop_nr,
                     struct char_data *keeper, struct char_data *buyer);
static int sell_price(struct obj_data *obj, int shop_nr,
                      struct char_data *keeper, struct char_data *seller);
static int ok_shop_room(int shop_nr, room_vnum room);
static int add_to_shop_list(struct shop_buy_data *list, int type, int *len,
                            int *val);
  ...
  do {
    ...
    if (!obj) {
      if (msg) {
        ...
        snprintf(buf, sizeof(buf), shop_index[shop_nr].no_such_item1,
                 GET_NAME(ch));
        do_tell(keeper, buf, cmd_tell, 0);
      }
      return (NULL);
...
static int sell_price(struct obj_data *obj, int shop_nr,
  ...
  float sell_cost_modifier = SHOP_SELLPROFIT(shop_nr) *
                             (1 - (GET_CHA(keeper) - GET_CHA(seller)) / 70.0);
...
static void shopping_buy(char *arg, struct char_data *ch,
  ...
  if (OBJ_FLAGGED(obj, ITEM_QUEST)) {
    ...
    if (buy_price(obj, shop_nr, keeper, ch) > GET_GOLD(ch) && !IS_GOD(ch)) {
      ...
      snprintf(actbuf, sizeof(actbuf), shop_index[shop_nr].missing_cash2,
               GET_NAME(ch));
      do_tell(keeper, actbuf, cmd_tell, 0);
  ...
  if (OBJ_FLAGGED(obj, ITEM_QUEST)) {
    while (obj && (GET_QUESTPOINTS(ch) >= GET_OBJ_COST(obj) || IS_GOD(ch)) &&
    ...
    while (
        obj &&
        (GET_GOLD(ch) >= buy_price(obj, shop_nr, keeper, ch) || IS_GOD(ch)) &&
        IS_CARRYING_N(ch) < CAN_CARRY_N(ch) && bought < buynum &&
        IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj) <= CAN_CARRY_W(ch)) {
      int charged;
  ...
  if (bought < buynum) {
    ...
    if (!obj || !same_obj(last_obj, obj))
      ...
      snprintf(buf, sizeof(buf), "%s You can only afford %d.", GET_NAME(ch),
             ...
             GET_QUESTPOINTS(ch) < GET_OBJ_COST(obj))
      snprintf(buf, sizeof(buf),
               "%s You only had sufficient quest points for %d.", GET_NAME(ch),
               bought);
  ...
  if (obj && OBJ_FLAGGED(obj, ITEM_QUEST))
    snprintf(tempbuf, sizeof(tempbuf), "%s That has cost you %d quest points.",
             GET_NAME(ch), goldamt);
  else
    snprintf(tempbuf, sizeof(tempbuf), shop_index[shop_nr].message_buy,
             GET_NAME(ch), goldamt);
...
static struct obj_data *get_selling_obj(struct char_data *ch, char *name,
  ...
  if (!(obj = get_obj_in_list_vis(ch, name, NULL, ch->carrying))) {
    if (msg) {
      ...
      snprintf(tbuf, sizeof(tbuf), shop_index[shop_nr].no_such_item2,
               GET_NAME(ch));
...
static void shopping_sell(char *arg, struct char_data *ch,
                          struct char_data *keeper, int shop_nr) {
  char tempstr[MAX_INPUT_LENGTH - 10], name[MAX_INPUT_LENGTH],
      tempbuf[MAX_INPUT_LENGTH]; // - 10 to make room for constants in format
  struct obj_data *obj;
...
static void shopping_value(char *arg, struct char_data *ch,
  ...
  snprintf(buf, sizeof(buf), "%s I'll give you %d gold coins for that!",
           GET_NAME(ch), sell_price(obj, shop_nr, keeper, ch));
  do_tell(keeper, buf, cmd_tell, 0);
}
...
static char *list_object(struct obj_data *obj, int cnt, int aindex, int shop_nr,
  ...
  snprintf(result, sizeof(result), " %2d)  %9s   %-*s %6d%s\r\n", aindex,
           quantity, count_color_chars(itemname) + 48, itemname,
...
static void shopping_list(char *arg, struct char_data *ch,
      ...
      if (CAN_SEE_OBJ(ch, obj) && GET_OBJ_COST(obj) > 0) {
        if (!last_obj) {
          ...
          if (!*name || isname(name, last_obj->name)) {
            strncat(buf,
                    list_object(last_obj, cnt, lindex, shop_nr, keeper, ch),
  ...
  if (!last_obj) /* we actually have nothing in our list for sale, period */
    ...
    if (has_quest)
      send_to_char(
          ch, "Items flagged \"qp\" require quest points to purchase.\r\n");
  }
}
...
static void read_line(FILE *shop_f, const char *string, void *data) {
  ...
  if (!get_line(shop_f, buf) || !sscanf(buf, string, data)) {
    log("SYSERR: Error in shop #%d, near '%s' with '%s'", SHOP_NUM(top_shop),
        buf, string);
    exit(1);
  }
}
  ...
  do {
    if (fgets(buf, sizeof(buf), shop_f) == NULL) {
      if (feof(shop_f))
        ...
        log("SYSERR: error reading reading shop file type list: %s",
            strerror(errno));
      else
...
static char *customer_string(int shop_nr, int detailed) {
  ...
  while (*trade_letters[sindex] != '\n' && len + 1 < sizeof(buf)) {
    if (detailed) {
      ...
      buf[len++] =
          (IS_SET(flag, SHOP_TRADE_WITH(shop_nr)) ? '_'
                                                  : *trade_letters[sindex]);
      buf[len] = '\0';
  ...
  *buf = '\0';
  for (shop_nr = 0; shop_nr <= top_shop && len < sizeof(buf); shop_nr++) {
    ...
    if (SHOP_KEEPER(shop_nr) == NOBODY)
      ...
      sprintf(buf1, "%6d",
              mob_index[SHOP_KEEPER(shop_nr)]
                  .vnum); /* sprintf: OK (for 'buf1 >= 11', 32-bit int) */
...
static void list_detailed_shop(struct char_data *ch, int shop_nr) {
  ...
  /* send_to_char(ch, "Customers:  %s\r\n", (ptrsave = customer_string(shop_nr,
   * TRUE)) ? ptrsave : "None"); */
...
bool shopping_identify(char *arg, struct char_data *ch,
                       struct char_data *keeper, int shop_nr) {
  ...
  if (!*arg) {
    snprintf(buf, sizeof(buf), "%s What do you want to identify??",
             GET_NAME(ch));
    do_tell(keeper, buf, cmd_tell, 0);
    return TRUE;
  }
  ...
  send_to_char(ch, "Weight: %d, Cost to Sell: %s%d%s, Cost to Buy: %s%d%s\r\n",
               GET_OBJ_WEIGHT(obj), QYEL, sell_price(obj, shop_nr, keeper, ch),
               QNRM, QYEL, buy_price(obj, shop_nr, keeper, ch), QNRM);
